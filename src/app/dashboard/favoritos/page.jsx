...
import { Star, Heart, Trash2, Filter, ImageOff } from "lucide-react"
...
export default function FavoritosPage() {
  ...
  const initialFavorites = [
    ...
    },
    {
      ...
      year: 2023,
      rating: 4.9,
      poster: "https://image.tmdb.org/t/p/w500/ncKCQVXgk3rK4GQrLI4YaSKzGIl.jpg",
    },
    {
      id: 3,
      title: "The Batman",
  ...
  const filteredFavorites =
      filter === "all" ? favorites : favorites.filter((movie) => movie.genre.toLowerCase().includes(filter.toLowerCase()))
        ...
        <div className="flex items-center justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="bg-gray-900 border-gray-800">
              <DropdownMenuItem onClick={() => setFilter("all")}>Todos los géneros</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setFilter("sci-fi")}>Ciencia Ficción</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setFilter("drama")}>Drama</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setFilter("acción")}>Acción</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setFilter("comedia")}>Comedia</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
              ...
              <Card key={movie.id} className="bg-gray-900 border-gray-800">
                <div className="flex p-4">
                  {!imageErrors[movie.id] ? (
                      ...
                      <div className="h-[150px] w-[100px] flex items-center justify-center bg-gray-800 rounded">
                        <div className="flex flex-col items-center text-gray-400">
                          <ImageOff className="h-8 w-8 mb-1" />
                          <span className="text-xs text-center px-2">No disponible</span>
                        </div>
                      </div>
                  )}
